...
export class SelectionTool extends PrimitiveTool {
        ...
        params: [
          {
            ...
            buttons: [
              ...
              { iconSpec: "icon-select-plus" },
              {
                ...
                isEnabledFunction: () => {
                  const tool = ExtensionHost.toolAdmin.activeTool;
  ...
  protected initSelectTool(): void {
    ...
    this.initLocateElements(
      enableLocate,
      false,
      enableLocate ? "default" : ExtensionHost.viewManager.crossHairCursor,
  ...
  private selectByPointsDecorate(context: any): void {
  ...
  public async selectDecoration(
    ...
    if (undefined === currHit)
      currHit = await ExtensionHost.locateManager.doLocate(
        new LocateResponse(),
        true,
        ev.point,
        ev.viewport,
    ...
    if (undefined !== currHit)
      return currHit.isElementHit
        ? ExtensionHost.viewManager.overrideElementButtonEvent(currHit, ev)
        : ExtensionHost.viewManager.onDecorationButtonEvent(currHit, ev);
  ...
  public override async onResetButtonUp(
    ...
    const lastHit =
      SelectionMode.Remove === this.selectionMode
        ? undefined
        : ExtensionHost.locateManager.currHit;
    if (lastHit && this.iModel.selectionSet.has(lastHit.sourceId)) {
      ...
      if (undefined === autoHit || autoHit.isSameHit(lastHit)) {
        ...
        let nextHit;
        do {
          nextHit = await ExtensionHost.locateManager.doLocate(
            response,
            false,
            ev.point,
            ev.viewport,
    ...
    if (
      EventHandled.Yes ===
      (await this.selectDecoration(ev, ExtensionHost.accuSnap.currHit))
    )
      return EventHandled.Yes;
  ...
  public override async onSuspend() {
    ...
    if (this.wantEditManipulators())
      ExtensionHost.toolAdmin.manipulatorToolEvent.raiseEvent(
        this,
  ...
  public override async onTouchMoveStart(
    ...
    if (startEv.isSingleTouch && !this._isSelectByPoints)
      await ExtensionHost.toolAdmin.convertTouchMoveStartToButtonDownAndMotion(
        startEv,
        ev
      );
  ...
  public override async onTouchMove(ev: BeTouchEvent): Promise<void> {
    if (this._isSelectByPoints)
      return ExtensionHost.toolAdmin.convertTouchMoveToMotion(ev);
  }
  public override async onTouchComplete(ev: BeTouchEvent): Promise<void> {
    if (this._isSelectByPoints)
      return ExtensionHost.toolAdmin.convertTouchEndToButtonUp(ev);
  }
  public override async onTouchCancel(ev: BeTouchEvent): Promise<void> {
    if (this._isSelectByPoints)
      return ExtensionHost.toolAdmin.convertTouchEndToButtonUp(
        ev,
        BeButton.Reset
      );
  }

  public override decorate(context: any): void {
  ...
  private syncSelectionMode(): void {
    ...
    if (this.wantToolSettings()) {
      ...
      ExtensionHost.toolAdmin.toolSettingsState.saveToolSettingProperty(
        this.toolId,
        syncMode
      );
  ...
  public override supplyToolSettingsProperties(): DialogItem[] | undefined {
    ...
    ExtensionHost.toolAdmin.toolSettingsState
